...
| `load_scenario`                   | `connector`, `file` or `scenario`              | Load scenario from file or inline (rejects a definition missing `id` / `name` / `targetType` / `nodes` / `edges`)                            |
| `list_scenarios`                  | `connector`                                    | List loaded scenarios (`scenarioId`, `name`, `active`, `state`, `mode`)                                                                      |
...
| `scenario_status`                 | `connector`, `scenarioId`                      | Get scenario execution status — keeps the terminal state (`completed` / `error`) + `runId` after the run ends, until the scenario is removed |
