...
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `data_transfer` | `{ "vendorId": string, "messageId"?: string, "data"?: string \| object }` | Send a station-initiated `DataTransfer.req` and **return the CSMS's answer** `{ "status", "data"? }` as the result (#348). `data` is a string, or an object: passed through on 2.0.1, JSON-encoded on 1.6 and 1.6S (whose `data` is a string). Rejects on CALLERROR, when the CALL is dropped (boot gate, socket closed, OCPP 1.2, which has no DataTransfer, and the simulator's 1.5 SOAP dialect, which does not send it), or after 30 s without an answer. Reachable over MCP through `call_method`. |
...
| `start_transaction` | `{ "connector": number, "tagId"?: string, "triggerReason"?: string, "chargingState"?: string }` | `connector` must be `>= 1`. `tagId` is optional since #299 — omitted, the charge point draws from its [idTag pool](#idtag-pool). `triggerReason` / `chargingState` (#335) shape the OCPP 2.x `TransactionEvent(Started)` — see [Transaction event parameters](#transaction-event-parameters); ignored on 1.6. |
| `stop_transaction` | `{ "connector": number, "reason"?: string, "triggerReason"?: string }` | `connector` must be `>= 1`. `reason` (#335) is one of the 1.6 `StopTransaction.req` or the 2.0.1 `stoppedReason` values, `triggerReason` the 2.x `TransactionEvent(Ended)` trigger — see [Transaction event parameters](#transaction-event-parameters). |
| `transaction_event` | `{ "connector": number, "triggerReason": string, "chargingState"?: string, "meterValues"?: boolean, "context"?: string }` | OCPP 2.x only (#335): a driven `TransactionEvent(Updated)` on the running transaction. On 1.6 the charge point logs a warning and sends nothing. See [Transaction event parameters](#transaction-event-parameters). |
...
| `firmware_status_notification` | `{ "status": string, "requestId"?: number }` | Send `FirmwareStatusNotification.req`. On 2.0.1 — and on 2.1, whose inbound registry and outbound handler are the 2.0.1 ones — the station **accepts** `UpdateFirmware` (it used to answer `Rejected`) and remembers its `requestId`; a report sent without one carries that id, an explicit one wins, and the status set is the full 2.0.1 `FirmwareStatusEnumType` (#345). Nothing is downloaded and, unlike 1.6, the 2.0.1 station does not walk the status machine on its own — the driver sends each report. On 1.6 `requestId` is dropped (the request has no such field) and the seven 1.6 statuses apply. |
| `log_status_notification` | `{ "status": string, "requestId"?: number }` | Send `LogStatusNotification.req` (1.6 Security Whitepaper / 2.0.1 N01). `status` is one of `BadMessage`, `Idle`, `NotSupportedOperation`, `PermissionDenied`, `Uploaded`, `UploadFailure`, `Uploading`, `AcceptedCanceled` (the last is 2.0.1-only). On 2.0.1 the station **accepts** `GetLog` with a `filename` and remembers its `requestId`, used when the command gives none; nothing is uploaded (#345). `PublishFirmware` stays refused with `reasonCode: NotLocalController` — the station is not a Local Controller, so `PublishFirmwareStatusNotification` is never sent. |
...
| `send_meter_value` | `{ "connector": number, "context"?: string }` | Send current meter value to the CSMS. `context` (#335) is the `ReadingContext` the sampled values carry (default `Sample.Periodic`); on 2.x it is now emitted as `sampledValue.context`. |
...
| `centralSystemUrl`, `soapPath`, `soapCallbackUrl` | string                           | SOAP only. `soapCallbackUrl` may be omitted when `server.info` reports a `soap.publicBaseUrl`: the daemon derives it and marks the CP's config `soapCallbackUrlDerived`. See [OCPP versions & transports](ocpp-versions-and-transports.md). |
...
Since #335 the OCPP 2.0.1 `TransactionEvent` builder no longer fixes
`triggerReason`, `stoppedReason` and `chargingState` per command. A caller
names them; the wire carries them verbatim; every existing caller that names
nothing gets the historical values. The vocabularies are the 2.0.1 enums in
full — one definition in `src/cp/domain/connector/Transaction.ts`, from which
the RPC schema, the JSON-Lines parser and the MCP tool schemas all derive.

| Parameter       | On                                                           | Values                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Default                                                               |
| --------------- | ------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| `triggerReason` | `start_transaction`, `stop_transaction`, `transaction_event` | `TriggerReasonEnumType`: `Authorized`, `CablePluggedIn`, `ChargingRateChanged`, `ChargingStateChanged`, `Deauthorized`, `EnergyLimitReached`, `EVCommunicationLost`, `EVConnectTimeout`, `MeterValueClock`, `MeterValuePeriodic`, `TimeLimitReached`, `Trigger`, `UnlockCommand`, `StopAuthorized`, `EVDeparted`, `EVDetected`, `RemoteStop`, `RemoteStart`, `AbnormalCondition`, `SignedDataReceived`, `ResetCommand`                                            | `Authorized` (Started), `StopAuthorized` (Ended); required on Updated |
| `chargingState` | `start_transaction`, `transaction_event`                     | `ChargingStateEnumType`: `Charging`, `EVConnected`, `SuspendedEV`, `SuspendedEVSE`, `Idle`                                                                                                                                                                                                                                                                                                                                                                        | `Charging` (Started); omitted on Updated                              |
| `reason`        | `stop_transaction`                                           | The 1.6 `StopTransaction.req` reason (`DeAuthorized`, `EmergencyStop`, `EVDisconnected`, `HardReset`, `Local`, `Other`, `PowerLoss`, `Reboot`, `Remote`, `SoftReset`, `UnlockCommand`) **or** the 2.0.1 `ReasonEnumType` (`EnergyLimitReached`, `GroundFault`, `ImmediateReset`, `LocalOutOfCredit`, `MasterPass`, `OvercurrentFault`, `PowerQuality`, `SOCLimitReached`, `StoppedByEV`, `TimeLimitReached`, `Timeout`, plus the eight names both spell the same) | `Local`                                                               |
| `meterValues`   | `transaction_event`                                          | boolean — attach the connector's current sampled values (`MeterValuesSampledData`, i.e. `SampledDataCtrlr/TxUpdatedMeasurands`) to the Updated event                                                                                                                                                                                                                                                                                                              | `false`                                                               |
| `context`       | `transaction_event`, `send_meter_value`                      | `ReadingContext`: `Sample.Periodic`, `Sample.Clock`, `Transaction.Begin`, `Transaction.End`, `Trigger`, `Interruption.Begin`, `Interruption.End`, `Other`                                                                                                                                                                                                                                                                                                         | `Sample.Periodic`                                                     |

How the two `reason` vocabularies meet: a 1.6 name sets the 1.6 `reason`
and, when 2.0.1 spells it identically, the explicit `stoppedReason` too; a
1.6-only name (`HardReset`, `SoftReset`, `UnlockCommand`) is mapped by the
2.0.1 encoder as before (`ImmediateReset`, `Reboot`, `Other`); a 2.0.1-only
name is sent verbatim on 2.x and becomes `Other` on a 1.6 `StopTransaction.req`,
which has no closer value. A value outside both vocabularies is an
`invalid_params` error that lists the accepted spellings.

Guarantees a conformance driver can rely on:

- `transaction_event` shares the per-transaction `seqNo` with the
  status-driven events, and a `chargingState` it names becomes the state the
  next status-driven `Updated` is de-duplicated against — a following
  `update_connector_status` to the same state sends nothing.
- `transaction_event` with no running transaction on the connector sends
  nothing and logs a warning; it never opens a transaction.
- `start_transaction { "triggerReason": "RemoteStart" }` is treated as a
  CSMS-initiated start and **bypasses the `AuthorizeBeforeLocalStart` gate**
  (#181) — the station sends no `Authorize` first. Every other trigger
  reason, `CablePluggedIn` included, is a local start and is gated.
- The periodic meter-value scheduler is **unchanged**: on 2.x it still emits a
  standalone `MeterValuesRequest`. Carrying scheduler samples inside
  `TransactionEvent(Updated)` by default is a wire-behaviour change for every
  existing 2.0.1 integration and is tracked separately; `transaction_event
{ "meterValues": true }` is the explicit way to produce that frame today.
...
| `server.info` | `{}` | Daemon facts a client needs before shaping a request: `version` and the SOAP public base (`soap.publicBaseUrl`, `soap.path`, `soap.tunnel`) a SOAP CP created without `soapCallbackUrl` derives one from (#183). |
| `events.subscribe` | `{ "scope": "*" \| "registry" \| "config" \| "scenario-definitions" \| "file-reload" \| "<cpId>" }` | Join event rooms and return an atomic snapshot; see [Event push and rooms](#event-push-and-rooms) for what each scope delivers. |
| `events.unsubscribe` | `{ "scope": "*" \| "registry" \| "config" \| "scenario-definitions" \| "file-reload" \| "<cpId>" }` | Leave an event room. |
