...
| `connector_runtime`  | Per-connector runtime state — status, availability, the active transaction (JSON), meter value, SoC and the last auto-started scenario / scenario position — keyed by `(cp_id, connector_id)`. Carries `soc_is_meter_derived` since schema v11 (#301): whether the stored SoC came off the energy register or was set explicitly, which decides whether the next transaction replaces it. Pre-v11 rows have no value and read as "set explicitly". |
