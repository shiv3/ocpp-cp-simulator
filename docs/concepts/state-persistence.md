...
| `watched_scenario_files` | Which file a scenario was loaded from, when a `load_scenario { file }` / `run_scenario_file` RPC loaded it (v13, #314). Read once at startup, after the fleet is restored, so `--watch` re-establishes the watch and reconciles an edit made while the daemon was down. Daemon-only, and deliberately not a column on `scenarios`: a source path means nothing to the browser services sharing that repository. Rows are dropped when the scenario is removed, replaced by a console upload, or found unloaded at restore.                                                                                                                          |
