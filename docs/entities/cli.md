...
8 charge points at a time. For SOAP, `--soap-public-base-url` (or the origin a
`--soap-tunnel` yields) is re-derived per generated id, while an explicit
`--soap-callback-url` must carry the same `{n}` placeholder — one callback
address cannot route a fleet.
...
| --------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Auto-meter time curve, session-relative                               | **Yes**, and agreeing: the curve is shifted so its value **at session start** (`t = 0`, not its earliest point) lands on the register the run started from, so a second session never rewinds and a curve beginning above zero is not double-counted. The trajectory between the points agrees too — see the row below.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Auto-meter curve shape between its points                             | **Yes**, since #329: both runtimes evaluate the same De Casteljau Bezier over all control points, from [one module](../../src/cli/exportK6/runtime/curve.ts) the bundle copies verbatim. The export used to interpolate piecewise-linearly, which agreed only on a two-point curve and from three points on ran ahead by up to ~15% of the curve's span at its midpoint. **A bundle regenerated after #329 can therefore send different MeterValues** than one generated before it, for a curve with three or more control points. A curve of three or more points **can** send different values, not always: the Bezier and the polyline coincide when the control points are evenly spaced on a straight line (a plain `(0,0) (10,10) (20,20)` ramp) or all carry the same value. Collinearity alone is not enough — `(0,0) (5,5) (20,20)` is collinear, unevenly spaced, and differs by 2.5 kWh at its widest. |
...
#### The subset, field by field

The tables below are the machine-checked half of the same boundary:
`src/cli/exportK6/__tests__/documentedSubset.test.ts` reads them out of this
page and compares them against `schema/scenario.schema.json` and the runtime's
own source, so a scenario node type or an `evSettings` field cannot be added to
the domain — or quietly implemented in the export — without this page moving
too. A missing or wrongly-marked row fails that test with a message naming the
field and the two ways out: implement it in the runtime, or record here that
the export does not (#329).

"Honoured" means the exported runtime reads the field, or walks the node type,
at all. It is not a claim that the behaviour matches the simulator's in every
detail — the table above is where those differences live.

| Scenario node type   | Honoured |
| -------------------- | -------- |
| `start`              | Yes      |
| `end`                | Yes      |
| `statusChange`       | Yes      |
...
| `notification`       | Yes      |
| `connectorPlug`      | Yes      |
| `remoteStartTrigger` | Yes      |
| `remoteStopTrigger`  | Yes      |
| `statusTrigger`      | Yes      |
| `reserveNow`         | Yes      |
| `cancelReservation`  | Yes      |
| `reservationTrigger` | Yes      |
| `statusNotification` | Yes      |
| `unlockOutcome`      | Yes      |
| `configSet`          | Yes      |
| `dataTransfer`       | Yes      |
| `csmsCallTrigger`    | Yes      |
| `responseOverride`   | Yes      |
| `inboundPolicy`      | No       |
| `certQuirks`         | No       |
...
| `evSettings` field   | Honoured                                                       |
| -------------------- | -------------------------------------------------------------- |
| `batteryCapacityKwh` | Yes — denominator of the target-SoC stop condition             |
| `initialSoc`         | Yes — opening SoC of the target-SoC stop condition             |
| `targetSoc`          | Yes — the target-SoC stop condition's threshold                |
| `maxChargingPowerKw` | Yes — default rate when the meterValue node sets no `outputKw` |
| `modelName`          | No — cosmetic; nothing on the wire carries it                  |
| `chargingCurve`      | No — the rate is the scenario's own increment or time curve    |
| `currentType`        | No — one measurand only, `Energy.Active.Import.Register`       |
| `phases`             | No — no per-phase samples                                      |
| `voltageV`           | No — no voltage or current samples                             |
| `powerFactor`        | No — no reactive-power model                                   |
...
| `--soap-tunnel <none\|ngrok>`       | No       | `none`                                  | SOAP versions, server mode only: expose the callback endpoint through a tunnel and derive the callback URL from its public origin (see [OCPP versions & transports → Tunnels](../concepts/ocpp-versions-and-transports.md#exposing-the-callback-through-a-tunnel)). Cannot be combined with `--soap-callback-url` / `--soap-public-base-url` — the tunnel is what provides the public base. Every SOAP charge point created without a callback URL (flags, RPC, console) derives one from it. |
| `--ngrok-auth-token <token>`        | No       | -                                       | ngrok authtoken, handed to the agent through its environment (`NGROK_AUTHTOKEN`) — never on its command line, never logged. Without it the agent uses its own config file or an inherited `NGROK_AUTHTOKEN`. Requires `--soap-tunnel ngrok`.                                                                                                                                                                                                                                                  |
| `--ngrok-domain <domain>`           | No       | -                                       | Reserved ngrok domain to request (`ngrok http --url`). Free-tier URLs change between runs. Requires `--soap-tunnel ngrok`.                                                                                                                                                                                                                                                                                                                                                                    |
| `--ngrok-api-url <url>`             | No       | -                                       | Attach to an already-running ngrok agent (e.g. a Docker sidecar, `http://ngrok:4040`) instead of spawning one; the https tunnel whose `addr` port is exactly `--soap-tunnel-port` is used; a tunnel to any other port is refused, even when it is the only one. Cannot be combined with the two options above. Requires `--soap-tunnel ngrok`.                                                                                                                                                |
| `--soap-tunnel-port <port>`         | No       | `0`                                     | Local port of the SOAP-only listener the tunnel forwards to; `0` picks a free one. Required with `--ngrok-api-url` (the running agent forwards to the port it was configured with). Requires `--soap-tunnel ngrok`.                                                                                                                                                                                                                                                                           |
