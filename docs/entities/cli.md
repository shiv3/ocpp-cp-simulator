...
| `--soap-tunnel <none\|ngrok>`       | No       | `none`                                  | SOAP versions, server mode only: expose the callback endpoint through a tunnel and derive the callback URL from its public origin (see [OCPP versions & transports → Tunnels](../concepts/ocpp-versions-and-transports.md#exposing-the-callback-through-a-tunnel)). An explicit `--soap-callback-url` / `--soap-public-base-url` wins and the tunnel is not started. |
