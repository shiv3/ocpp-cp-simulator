...
| `--soap-tunnel-port <port>`         | `0`                          | Local port of the SOAP-only listener the tunnel forwards to (`0`: a free port). Required with `--ngrok-api-url`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
simulator does not add a non-standard shared secret to SOAP payloads. With
`--soap-tunnel` the same route is also served by a dedicated listener
(`--soap-tunnel-port`) that the tunnel forwards to: that listener answers `404`
to every other path and has no Basic-auth gate — the CSMS is its only client —
see [OCPP versions & transports → Tunnels](../concepts/ocpp-versions-and-transports.md#exposing-the-callback-through-a-tunnel).
