...
- **A symlink is watched at both ends.** The daemon watches the directory
  ...
  way this could look supported and not be.
...
| A **broken** symlink (target not there yet), or a symlink **cycle**       | Yes       | Same mechanism, and through the whole chain: every hop up to and including the first one that does not exist has its directory watched, so the creation is seen wherever it lands. Not treated as a degraded filesystem, so it does not consume the one-off degradation line. |
