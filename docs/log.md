...
## [2026-09-12] ingest | OCPP 2.0.1 TransactionEvent reaches the width the 1.6 builder has (#335)

- Asked for by the maintainer of open-ocpp-tck (juherr/open-ocpp-tck#114): the 2.0.1 builder fixed `triggerReason` per command (`Authorized`, `ChargingStateChanged`, `StopAuthorized`), `chargingState` to three values, `stoppedReason` to whatever the 1.6 `stopReason` mapped to (always `Local` from a driven station), and computed `sampledValue.context` then dropped it — so ~24 mandatory CSMS conformance cases named for those values could not be driven. A capability gap, not a defect.
- [Control plane](concepts/control-plane.md#transaction-event-parameters): the new canonical table. `start_transaction` gains `triggerReason` / `chargingState`; `stop_transaction` gains `reason` (1.6 ∪ 2.0.1 vocabularies, one parameter) and `triggerReason`; `transaction_event` is a new CP command for a driven `Updated` event, optionally carrying the connector's sampled values with a `context`; `send_meter_value` gains `context`. The vocabularies are defined once in `src/cp/domain/connector/Transaction.ts` and the RPC schema, the JSON-Lines parser and the MCP tool schemas derive from them — the #316 lesson applied before the drift could start. The three surfaces were changed together and `mcpToolSchemaParity` pins tool ≥ method.
- Decision recorded on the page: the periodic scheduler still emits standalone `MeterValuesRequest` on 2.x. Making scheduler samples ride `TransactionEvent(Updated)` by default changes the wire for every existing 2.0.1 integration, so it is a named follow-up; `transaction_event { meterValues: true }` produces that frame explicitly today.
- Wire changes for callers that name nothing: exactly one — 2.x `MeterValues.sampledValue[].context` is now present (`Sample.Periodic`). Started/Ended defaults are unchanged and pinned by the existing `v201Transaction` suite; the new `v201TransactionEventWidth` suite pins the widened values, the seqNo sharing and de-dup interaction of driven Updated events, the no-transaction no-op, and the 1.6 warn-and-send-nothing path.
- [MCP endpoint](entities/mcp-endpoint.md#curated-tools) tool table and [GitHub issues](sources/github-issues.md) updated.
