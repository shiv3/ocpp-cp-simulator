...
| `logs.get`           | `{ "cpId": string, "limit"?: number, "offset"?: number, "order"?: "asc" \| "desc" }`                                                                                                                  | Return persisted logs, or the in-memory log buffer when no state DB is configured. `limit` takes the **most recent** N; see [Log windowing](#log-windowing). |
...
| `get_logs`              | `logs.get`                | `cpId`, `limit?`, `offset?`, `order?` — `limit` takes the most recent N                           |
