...
export function soapTunnelStartupLines(
  ...
  return [
    ...
    `The tunnel forwards to the SOAP-only listener on port ${tunnel.localPort}: the callback route ` +
      "is the only one it publishes; socket.io, the web console and MCP stay local.",
  ];
}
