...
export function runMigrations(db: Database): void {
  ...
  if (stored < 13) {
    ...
    // Second half of the same feature: the scenario file a control-plane
    // `load_scenario { file }` / `run_scenario_file` was loaded from, so its
    // watch is re-established on restart. The startup flags (`--scenario`,
    // `--scenario-template-file`) re-register by themselves because the
    // bootstrap runs again; only the control-plane loads were lost.
    db.exec(
      "CREATE TABLE IF NOT EXISTS watched_scenario_files (" +
